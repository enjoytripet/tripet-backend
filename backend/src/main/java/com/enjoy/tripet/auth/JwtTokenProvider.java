...
import com.enjoy.tripet.dtos.TokenInfo;
import com.enjoy.tripet.exception.ErrorCode;
import com.enjoy.tripet.exception.TripetException;
...
public class JwtTokenProvider {
    ...
    public Authentication getAuthentication(String accessToken) {
        ...
        if (claims.get("auth") == null) {
            throw new TripetException(ErrorCode.UNAUTHORIZED_TOKEN);
        }
        ...
        PrincipalDetails principal = memberRepository.findByEmail(claims.getSubject())
                ...
                .orElseThrow(()-> new TripetException(ErrorCode.MEMBER_NOT_FOUND));
    ...
    public boolean validateToken(String token) {
        try {
            ...
            throw new TripetException(ErrorCode.INVALID_TOKEN);
        } catch (UnsupportedJwtException e) {
            log.info("Unsupported JWT Token", e);
            throw new TripetException(ErrorCode.UNSUPPORTED_TOKEN);
        } catch (IllegalArgumentException e) {
            log.info("JWT claims string is empty.", e);
            throw new TripetException(ErrorCode.EMPTY_CLAIM_TOKEN);
        } catch (ExpiredJwtException e){
            log.error("Expired JWT Token", e);
            throw new TripetException(ErrorCode.EXPIRED_TOKEN);
        }
    }
